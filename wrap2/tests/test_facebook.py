...
class FacebookTestCase(unittest.TestCase):
    """TestCase for Facebook wrapper"""

    # mocking data returned by Facebook
    get_data = r'''{"data":[{"name":"posts","fql_result_set":[{"post_id":"302590276513997_316451585127866",
    ...
    "name":"Yami Malik Ishtar", "pic_small_with_logo": "http://pic.example.com"},
    {"uid":100001964824717,"name":"Kitana Rayna Webb", "pic_small_with_logo": "http://pic.example.com"}]},
    {"name":"comment_actor_info","fql_result_set":[{"uid":100000103765943,"name":"Yami Malik Ishtar",
    "pic_small_with_logo": "http://pic.example.com"},
    {"uid":100003032215825,"name":"Kairis Namine","pic_small_with_logo": "http://pic.example.com"}]}]}'''

    get_data_expected = [{'created_at': datetime.datetime(2012, 12, 17, 20, 50, 7),
                          'entities': {'urls': [{
                          ...
                          'from_user_name': 'Kitana Rayna Webb',
                          'geo': None,
                          ...
                          'profile_image_url': 'http://pic.example.com',
                          'source': 'Facebook',
                         ...
                         {'created_at': datetime.datetime(2012, 12, 17, 23, 6, 19),
                          ...
                          'from_user_name': 'Yami Malik Ishtar',
                          'geo': None,
    ...
    def test_get(self):
        ...
        (mockito.when(facebook_api.GraphAPI).request(mockito.any(), args=mockito.any())
            .thenReturn(anyjson.loads(self.get_data)))

        fb = facebook.Facebook(1, 2)
        results = fb.get(dict(ids=[123123123, 234234234]), 'sometesttoken')

        self.assertEqual(results, self.get_data_expected)
    ...
    def test_get_authorization_url(self):
        ...
        self.assertTrue(url.startswith(fb.FB_OATH_BASE))
        self.assertEqual(urlparse.parse_qs(urlparse.urlparse(url).query), {'scope': ['read,write'],
